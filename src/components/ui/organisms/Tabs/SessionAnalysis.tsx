...
export const SessionAnalysis: React.FC<SessionAnalysisProps> = ({ trips, onClear }) => {
    ...
    return (
      <div className="glass-card rounded-4xl p-8 text-center border-dashed border-white/10 animate-in fade-in duration-500">
        <div className="relative w-16 h-16 bg-white/5 rounded-2xl flex items-center justify-center mx-auto mb-4">
          <div className="absolute inset-0 bg-info/10 blur-xl rounded-full" />
        ...
        <p className="text-xs text-white/20 uppercase font-bold mt-2 tracking-widest">
      ...
      <div className="glass-card rounded-4xl p-6 animate-in slide-in-from-bottom-2 duration-500">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-xs font-black uppercase tracking-[0.2em] text-white/60">Calentando Motores</h3>          </div>
          <div className="px-3 py-1 bg-white/5 rounded-full border border-white/10">
            <span className="text-xs font-black text-white/40 uppercase">{trips.length}/3 VIAJES</span>
          </div>
        </div>
        ...
        <button
          ...
          className="w-full mt-4 py-2 text-xs font-black text-white/20 uppercase tracking-[0.3em] hover:text-white transition-colors"
        >
  ...
  return (
    <div className="glass-card rounded-4xl overflow-hidden shadow-2xl border border-white/5 animate-in fade-in duration-500">

      {/* 2. HEADER: Sistema de Niveles y XP (NUEVO) */}
      <div className="px-6 pt-6 pb-5 bg-linear-to-b from-white/3 to-transparent">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 bg-warning/20 rounded-xl flex items-center justify-center border border-warning/30">
              <Trophy className="w-5 h-5 text-warning" />
            </div>
            <div>
              <h3 className="text-xs font-black uppercase tracking-widest text-white italic">Radar manejate</h3>
              <p className="text-xs text-white/30 font-bold uppercase tracking-widest">Balance de la jornada</p>            </div>
          </div>

          <button
            onClick={() => confirm('¿Limpiamos el balance del día?') && onClear()}
            className="p-2 text-white/10 hover:text-error transition-colors rounded-full hover:bg-white/5"
          >
        ...
        <div className="space-y-2">
          <div className="flex justify-between items-end">
            <span className="text-xs font-black text-warning italic">RANGO {insights.driverLevel}</span>
          ...
          <div className="h-1.5 w-full bg-white/5 rounded-full overflow-hidden p-px">
            <div
              className="h-full bg-linear-to-r from-warning/80 to-warning/40 rounded-full transition-all duration-1000"
      ...
      <div className="px-6 pb-6">
        <div className="grid grid-cols-3 gap-3">
          <div className="bg-white/2 p-4 rounded-3xl border border-white/5 text-center group hover:bg-white/4 transition-colors">
            <Target className="w-4 h-4 text-success mx-auto mb-2 opacity-50" />
            <p className="text-2xl font-black text-success italic tracking-tighter">{insights.profitableTripsPercent}%</p>
            <p className="text-xs text-white/20 uppercase font-black tracking-widest">Puntería</p>
          </div>

          <div className="bg-white/2 p-4 rounded-3xl border border-white/5 text-center group hover:bg-white/4 transition-colors">
            <div className="flex justify-center mb-2">
              {insights.trend === 'improving' && <TrendingUp className="w-4 h-4 text-info" />}
              {insights.trend === 'declining' && <TrendingDown className="w-4 h-4 text-error" />}
            ...
            <p className="text-xs text-white/20 uppercase font-black tracking-widest">El rumbo</p>
          </div>

          <div className="bg-white/2 p-4 rounded-3xl border border-white/5 text-center group hover:bg-white/4 transition-colors">
            <Zap className="w-4 h-4 text-warning mx-auto mb-2 opacity-50" />
            <p className="text-2xl font-black text-warning italic tracking-tighter">{insights.profitableStreak}</p>
            <p className="text-xs text-white/20 uppercase font-black tracking-widest">Seguidilla</p>
          </div>
        </div>
      </div>
      ...
      {insights.badges.length > 0 && (
        <div className="px-6 pb-6">
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="text-xs font-black text-white/20 uppercase tracking-[0.2em]">Medallas del día</span>          </div>
            ...
            {insights.badges.map((badge) => (
              ...
              >
                ...
                <div className="flex flex-col">
                  <span className="text-xs font-black uppercase tracking-tighter whitespace-nowrap">{badge.name}</span>
                  <span className="text-xs opacity-60 font-bold uppercase">{badge.description}</span>
                </div>
              </div>
            ))}
          </div>
      ...
      >
        <div className="flex items-center gap-2">
          <BrainCircuit className="w-4 h-4 text-info" />
          <span className="text-xs font-black text-white/40 uppercase tracking-[0.3em] group-hover:text-white transition-colors">
      ...
      {/* 6. TIPS Y COMPARATIVA (Expandible con PremiumGate) */}
      {isExpanded && (
          ...
          <div className="border-t border-white/5 px-6 py-8 space-y-8 bg-black/20 animate-in slide-in-from-top-4 duration-500">
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-2 ml-1">
                ...
                <p className="text-xs font-black text-white/20 uppercase tracking-[0.2em]">Consejos</p>            </div>
                ...
                {insights.tips.map((tip, i) => (
                  <div key={i} className="flex gap-4 p-5 bg-white/2 rounded-3xl border border-white/5 items-start">
                    <div className="w-6 h-6 rounded-lg bg-warning/10 flex items-center justify-center shrink-0">
                      <span className="text-xs font-black text-warning">{i + 1}</span>
                    </div>
            ...
            <div className="p-6 bg-linear-to-br from-info/20 to-info/5 rounded-4xl border border-info/30 text-center relative overflow-hidden">
