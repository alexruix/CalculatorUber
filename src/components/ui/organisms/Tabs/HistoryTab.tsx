                ...
                <div className="glass-card rounded-4xl p-6 border border-white/5 shadow-2xl relative overflow-hidden">
                    ...
                    <div className="flex items-center justify-between mb-6 relative">
                        ...
                        <button
                            ...
                            className="w-10 h-10 bg-error/10 border border-error/20 rounded-xl flex items-center justify-center text-error active:scale-90 transition-all"
                        >
                        ...
                        <div key={date} className="space-y-4">
                            ...
                            <div className="space-y-3 relative">
                                ...
                                {data.trips.map((trip) => {
                                    ...
                                    const isTraffic = (trip.avgSpeed && trip.avgSpeed > 0 && trip.avgSpeed < 20) || false;
                                    const isConfirmingDelete = deletingId === trip.id;

                                    const cardBorder = isLoss
                                        ? 'border-error/30 bg-error/2'
                                        : isTraffic
                                            ? 'border-warning/30 bg-warning/5'
                                            : 'border-white/5';

                                    return (
                                        <div key={trip.id} className="relative space-y-3">
                                            <div className={`glass-card rounded-2xl p-4 border transition-all duration-300 flex items-center justify-between group ${cardBorder}`}>
                                                    ...
                                                    <div>
                                                        ...
                                                        <div className="flex items-center gap-3 mt-1 opacity-30">
                                                            ...
                                                            {trip.vertical && (
                                                                <div className="flex items-center gap-1 ml-auto">
                                                                    {trip.vertical === 'delivery' && <Bike className="w-3 h-3 text-info" />}
                                                                    {trip.vertical === 'logistics' && <Truck className="w-3 h-3 text-info" />}
                                                                    {trip.vertical === 'transport' && <CarIcon className="w-3 h-3 text-info" />}
                                                                </div>
                                                            )}
                                                        </div>
                                                        ...
                                                        {(trip.tip || trip.tolls) && (
                                                            <div className="flex items-center gap-3 mt-1.5 pt-1.5 border-t border-white/5">
                                                                {(trip.tip ?? 0) > 0 && (
                                                                    <div className="flex items-center gap-1">
                                                                        <Coins className="w-2.5 h-2.5 text-success" />
                                                                        <span className="text-[10px] font-black uppercase text-success">Propina: ${trip.tip}</span>
                                                                    </div>
                                                                )}
                                                                {(trip.tolls ?? 0) > 0 && (
                                                                    <div className="flex items-center gap-1">
                                                    ...
                                                    <div className="flex items-center">
                                                        {isConfirmingDelete ? (
                                                            <div className="flex items-center gap-1 animate-in slide-in-from-right-2">
                                                                ...
                                                                <button onClick={() => onDeleteTrip(trip.id)} className="bg-error text-white p-2 rounded-lg active:scale-90"><Check className="w-4 h-4" /></button>
                                                            </div>
                                                        ) : (
                                                            <button onClick={() => setDeletingId(trip.id)} className="p-2 text-white/5 hover:text-error transition-colors opacity-0 group-hover:opacity-100 touch-target"><Trash2 className="w-4 h-4" /></button>
                                                        )}
                                                    </div>

                                                    {/* Badge de velocidad / tráfico */}
                                                    {(trip.avgSpeed && trip.avgSpeed > 0) ? (
                                                        <div className={`flex items-center gap-1 px-2 py-0.5 rounded-full border text-[10px] font-black uppercase tracking-widest ${isTraffic ? 'border-warning/30 text-warning bg-warning/10' : 'border-white/10 text-white/30 bg-white/5'}`}>
                                                            {isTraffic && <AlertTriangle className="w-2.5 h-2.5" />}
                                                            <span>{Math.round(trip.avgSpeed)} km/h</span>
                                                        </div>
                                                    ) : null}
                                                </div>
                                            </div>

                                            {/* Separador visual de tiempo de espera (si hubo espera antes de este viaje) */}
                                            {(trip.waitMinutes && trip.waitMinutes > 0) ? (
                                                <div className="flex justify-center py-1 relative z-10 w-full mb-3">
                                                    <div className="bg-black border border-white/5 px-3 py-1 rounded-full flex items-center gap-2">
                                                        <Clock className="w-3 h-3 text-white/30" />
                                                        <span className="text-[10px] font-bold text-white/40 uppercase tracking-widest">
                                                            {trip.waitMinutes} min de espera
                                                        </span>
                                                    </div>
                                                </div>
                                            ) : null}
