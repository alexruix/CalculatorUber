...
export const GoogleButton = React.forwardRef<HTMLButtonElement, GoogleButtonProps>(
    ({ onSuccessRoute = '/app', showText = true, className, disabled, ...props }, ref) => {
        ...
        return (
            <Button
                ...
                onClick={(e) => {
                    e.preventDefault();
                    e.stopPropagation();
                    handleGoogleLogin(`${window.location.origin}${onSuccessRoute}`);
                }}
                disabled={isDisabled}
                className={cn("flex-1 gap-3 h-14", className)}
            ...
            >
                ...
                {showText && !loading && <span>Google</span>}
                {showText && loading && <span>Google</span>}
            </Button>
        );
    }
